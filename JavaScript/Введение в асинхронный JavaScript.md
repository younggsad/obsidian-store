...
1. Получаем ссылку на элемент [`<button>`](https://developer.mozilla.org/ru/docs/Web/HTML/Reference/Elements/button), который уже есть в DOM.
2. Добавляем к кнопке обработчик события [`click`](https://developer.mozilla.org/ru/docs/Web/API/Element/click_event) так, что при нажатии на неё:
    1. Выводится сообщение [`alert()`](https://developer.mozilla.org/ru/docs/Web/API/Window/alert).
    2. После закрытия сообщения создаём элемент [`<p>`](https://developer.mozilla.org/ru/docs/Web/HTML/Reference/Elements/p) (абзац).
...
**Примечание:** Важно помнить, что [`alert()`](https://developer.mozilla.org/ru/docs/Web/API/Window/alert), хоть и часто используется для демонстрации синхронных блокирующих операций, сильно не рекомендован к использованию в реальных приложениях.
...
Есть два типа стиля асинхронного кода, с которыми вы столкнётесь в коде JavaScript, старый метод — колбэки (callbacks) и более новый — промисы ([[Promise]]). В следующих разделах мы познакомимся с каждым из них.
...
Пример асинхронного колбэка вторым параметром [`addEventListener()`](https://developer.mozilla.org/ru/docs/Web/API/EventTarget/addEventListener "addEventListener()") (как мы видели выше):
...
Вы можете написать свою собственную функцию, содержащую колбэк-функцию. Давайте взглянем на ещё один пример, в котором происходит загрузка ресурсов через [`XMLHttpRequest` API](https://developer.mozilla.org/ru/docs/Web/API/XMLHttpRequest):
...
Мы создали функцию `displayImage()`, которая представляет blob, переданный в неё, как объект URL, и создаёт картинку, в которой отображается URL, добавляя её в элемент документа `<body>`. Однако, далее мы создаём функцию `loadAsset()`, которая принимает колбэк-функцию в качестве параметра, вместе с URL для получения данных и типом контента. Для получения данных из URL используется `XMLHttpRequest` (часто сокращается до аббревиатуры "XHR") , перед тем как передать ответ в колбэк-функцию для дальнейшей обработки. В этом случае колбэк-функция ждёт, пока XHR закончит загрузку данных (используя обработчик события [`onload`](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequestEventTarget/load_event)) перед отправкой данных в колбэк-функцию.
...
Заметьте, что не все колбэк-функции асинхронны — некоторые запускаются синхронно. Например, при использовании [`Array.prototype.forEach()`](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Array/forEach) для перебора элементов массива ([запустите пример](https://mdn.github.io/learning-area/javascript/asynchronous/introducing/foreach.html "Внешняя ссылка (открывается в новой вкладке)"), и [посмотрите исходный код](https://github.com/mdn/learning-area/blob/master/javascript/asynchronous/introducing/foreach.html "Внешняя ссылка (открывается в новой вкладке)")):
...
Промисы — новый стиль написания асинхронного кода, который используется в современных Web API. Хорошим примером является [`fetch()`](https://developer.mozilla.org/ru/docs/Web/API/Window/fetch) API, который современнее и эффективнее чем [`XMLHttpRequest`](https://developer.mozilla.org/ru/docs/Web/API/XMLHttpRequest). Посмотрим на краткий пример, из нашей статьи [Fetching data from the server](https://developer.mozilla.org/ru/docs/Learn_web_development/Core/Scripting/Network_requests):
...
**Примечание:** Вы можете посмотреть законченную версию на Github ([посмотрите исходный код](https://github.com/mdn/learning-area/blob/master/javascript/apis/fetching-data/can-store/can-script.js "Внешняя ссылка (открывается в новой вкладке)") и [запустите пример](https://mdn.github.io/learning-area/javascript/apis/fetching-data/can-store/ "Внешняя ссылка (открывается в новой вкладке)")).
...
Может понадобиться много времени, чтобы привыкнуть к данной концепции; это немного напоминает [Кота Шрёдингера](https://ru.wikipedia.org/wiki/%D0%9A%D0%BE%D1%82_%D0%A8%D1%80%D1%91%D0%B4%D0%B8%D0%BD%D0%B3%D0%B5%D1%80%D0%B0 "Внешняя ссылка (открывается в новой вкладке)") в действии. Ни один из возможных результатов ещё не произошёл, поэтому операция fetch в настоящее время ожидает результата. Далее у нас есть три блока кода следующих сразу после `fetch()`:
...
**Примечание:** Из соображений безопасности вы не можете применять `fetch()` к файлам из вашей локальной системы (или запустить другие такие операции локально); чтобы запустить локально пример выше вам необходимо запустить его через [локальный веб-сервер](https://developer.mozilla.org/ru/docs/Learn_web_development/Howto/Tools_and_setup/set_up_a_local_testing_server).
